...
struct ContentView: View {
    ...
    var body: some View {
        ...
        if isAuthenticated  {
            FetchUser(email: self.username, password: self.password )
        }else{
            NavigationView{
                ZStack {
                    Rectangle().fill(.black)
                    LinearGradient(colors: [Color.blue.opacity(0.4),Color.cyan.opacity(0.2)],startPoint: .topLeading,endPoint: .bottomTrailing).ignoresSafeArea()
                    ...
                    VStack {
                        Text("Closer Together").font(.system(size: 38,weight: .bold,design: .rounded)).foregroundStyle(LinearGradient(colors: [.accentColor,.purple], startPoint: .topLeading, endPoint: .bottomTrailing))
                        ...
                        NavigationLink(destination: SignUpWindow()) {
    ...
    func authenticateUser(_ user: String, _ password: String) {
        ...
        guard let url = URL(string: "\(Constants.baseURL)couplesUser/userCouple?email=\(username)&password=\(password)") else {
            return
        }
...
//    func logUserr(_ username:String, _ password:String){
//        
//        guard let url = URL(string: "\(Constants.baseURL)couplesUser/userCouple?email=\(username)&password=\(password)") else {
//            return
//        }
